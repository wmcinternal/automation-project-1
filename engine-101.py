...
def extract_currency_matrix_table(pdf, search_currencies, share_class_target):
    ...
    for page in pdf.pages:
        ...
        for table in tables:
            ...
            target = share_class_target.strip().lower()

            if len(table) == 1 or all(len(row) <= 1 for row in table):
                single_text = " ".join([str(c) for row in table for c in row if c]).lower()
                
                if any(ik in single_text for ik in investment_keywords) and bool(re.search(rf"\bclass\s*{target}(?:[\s\)]|$)", single_text)):
                    ccy_pattern = "|".join(search_currencies)
                    
                    # Match class block and dynamically capture the FIRST currency value after target class
                    # Matches "Class A ... HKD 10,000" before hitting "Class B" or "Class C"
                    class_block_match = re.search(
                        rf"\bclass\s*{target}\b.*?(?:({ccy_pattern})\s*([\d,]+(?:\.\d+)?))", 
                        single_text, 
                        re.DOTALL | re.IGNORECASE
                    )
                    if class_block_match:
                        val = float(class_block_match.group(2).replace(',', ''))
                        if val > 0.0:
                            return val, val
            ...
            for r_idx, row in enumerate(table):
                for col_idx, cell_text in enumerate(row):
                    ...
                    is_match = (cell_clean == target) or bool(re.search(rf"\bclass\s*{target}\b", cell_clean))
                    print(f"   🔎 [S2 TRACE Row {r_idx} Col {col_idx}] cell_clean: {repr(cell_clean)} | target: {repr(target)} | is_match: {is_match}")
                    if is_match:
                        matched_class_col = col_idx
                        header_row_idx = r_idx
                        print(f"   🎯 [S2 TRACE] MATCH LOCKED on Col {col_idx} (Header Row {r_idx})")
                        break
            ...
            if matched_class_col != -1:
                ...
                for row in table[header_row_idx + 1:]:
                    ...
                    print(f"   📝 [S2 TRACE Row ] Cell Value below header:\n{repr(cell_val)}")
                    match = re.search(rf"(?:{ccy_pattern})\s*([\d,]+(?:\.\d+)?)", cell_val, re.IGNORECASE)
                    if match:
                        val = float(match.group(1).replace(',', ''))
                        print(f"   ✅ [S2 TRACE] Regex matched currency value: {val}")
                        if val > 0.0:
                            return val, val
                    else:
                        print(f"   ❌ [S2 TRACE] Regex pattern r'{ccy_pattern}' failed on cell_val")
    return 0.0, 0.0
