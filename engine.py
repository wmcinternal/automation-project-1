...
import pdfplumber
from api import extract_with_ai, convert_pdf_to_base64_images, extract_markdown_with_docling, extract_table_crop
                ...
                for row in table[header_row_idx + 1:]:
                    ...
                    if any(ccy in row_text for ccy in search_currencies):
                        ...
                        if found_amounts:
                            ...
                            val_init = nums[0]
                            val_sub = nums[1] if len(nums) > 1 else nums[0]
...
def extract_pdf_metrics(matched_pdf, target_currency, target_fund, folder="."):
    ...
    try:
        with pdfplumber.open(os.path.join(folder, matched_pdf)) as pdf:
            ...
            if min_int==0.0 and min_sub==0.0:
                print(f"🔄 [Regex Failed] Falling back to api.py for {matched_pdf}...")
                pdf_full_path=os.path.join(folder, matched_pdf)

                pdf_images=convert_pdf_to_base64_images(pdf_full_path)
                docling_md=extract_markdown_with_docling(pdf_full_path)
                truncated_context=docling_md[:5000] if docling_md else ""

                ai_res=extract_with_ai(
                    markdown_context=truncated_context,
                    images=pdf_images,
                    target_class=share_class_target.upper(),
                    target_currency=target_currency.upper()
                )

                if ai_res and isinstance(ai_res, dict):
                    class_data = ai_res.get(share_class_target.upper(), {})
                    min_int = float(class_data.get("Min Int Amt", 0.0))
                    min_sub = float(class_data.get("Min Sub Amt", 0.0))
