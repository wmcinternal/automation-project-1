...
def extract_transaction_grid_table(pdf, search_currencies, share_class_target):
    ...
    class_pattern = rf"\bclass\s*{share_class_target}"
            ...
            for row_idx, row in enumerate(table):        
                ...
                if is_section_header:
                    ...
                    for i, cell in enumerate(clean_row):
                        ...
                        '''
                        if any(term in cell for term in ["initial", "minimum subscription amount", "minimum investment"]):
                            col_idx_initial = i
                        elif any(term in cell for term in ["additional", "subsequent"]):
                            col_idx_additional = i
            ...
            if matched_class_col != -1:
                for row in table[header_row_idx:]:
                    ...
                    if any(ccy in row_text for ccy in search_currencies):
                        ...
                        # 1. Identify which column index our target share class occupies within the cell
                        # In this PDF, the line "Share class A C" means A is index 0 and C is index 1.
                        target_sub_idx = 0
                        for line in lines:
                            if "share class" in line.lower():
                                class_labels = [p.upper() for p in line.split() if len(p) == 1 and p.isalpha()]
                                if share_class_target.upper() in class_labels:
                                    target_sub_idx = class_labels.index(share_class_target.upper())
                                break

                        val_init = 0.0
                        val_sub = 0.0
                        
                        # 2. Extract Initial Amount
                        for line in lines:
                            if any(ccy in line.lower() for ccy in search_currencies):
                                found = re.findall(rf"(?:{ccy_pattern})\s*([\d,]+(?:\.\d+)?)", line, re.IGNORECASE)
                                if found and len(found) > target_sub_idx:
                                    if val_init == 0.0: # Capture the first valid currency amount
                                        val_init = float(found[target_sub_idx].replace(',', ''))
                        
                        # 3. Extract Subscription (Additional) Amount
                        for line in reversed(lines):
                            if "additional" in line.lower() or "-" in line:
                                # Check for multiple dashes (one per share class)
                                dashes = re.findall(r"-", line)
                                if len(dashes) > target_sub_idx:
                                    val_sub = 0.0 # It's a dash in the PDF
                                else:
                                    val_sub = parse_amount(line, search_currencies)
                                break
                        
                        if val_init > 0 or val_sub > 0:
...
def extract_docling_to_table(pdf, search_currencies, share_class_target):
    return 0.0, 0.0
...
if __name__ == "__main__":
    company_data = load_target_data("testing_api.xlsx")
